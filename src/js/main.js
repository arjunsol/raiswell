...
class SiteManager {
    ...
    /**
     * Initialize hero slider with config from XML
     */
    initializeHeroSlider() {
        const heroSliderConfig = window.siteConfig.getHeroSlider();
        if (heroSliderConfig && heroSliderConfig.slides && heroSliderConfig.slides.length > 0) {
            HeroSlider.init(heroSliderConfig);
        } else {
            console.warn('Hero slider: No slides configured');
        }
    }

    /**
     * Initialize testimonials slider with config from XML
     */
    initializeTestimonialsSlider() {
        const testimonials = window.siteConfig.getTestimonials();
        ...
        if (testimonialsContainer && testimonials && testimonials.length > 0) {
            new TestimonialsSlider(testimonialsContainer, testimonials.map(t => ({
                text: t.text,
